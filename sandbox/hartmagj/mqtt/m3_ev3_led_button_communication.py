...
# d: 2. Within the MyDelegate class below add the method, set_led, to receive messages as described above.
...
class MyDelegate(object):
    ...
    def set_led(self, side, color):
        print("Received: {} {}".format(side, color))
        led_side = None
        if side == "left":
            led_side = ev3.Leds.LEFT
        elif side == "right":
        ...
        if led_side is None or led_color is None:
            print("Invalid parameters sent to set_led. led_side_string = {} led_color_string = {}".format(side, color))
        else:
            ev3.Leds.set_color(led_side, led_color)
